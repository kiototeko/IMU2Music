...
In this table they show their validation loss compared to other methods, according to different musical instruments. We are only interested on the last line and on the violin result as our model is also trained to deal with violinists performing. One thing to note from this table is that we were not able to replicate their results, at least the ones corresponding to the violin, even when using their exact model cloned from their repository with almost no change. That said, we present now our results:
